...
DEFAULT_CURSOR_MODEL = "cursor-default"
...
def _call_cursor(
    ...
    """调用 Cursor 代理的 OpenAI 兼容 chat/completions 接口。"""
...
def get_test_scope_suggestion(
    ...
    Args:
        ...
        model: 可选，模型名，默认 cursor-default。
        project_context: 可选，与变更相关的项目 depot 区域结构，便于从全项目角度给出测试范围。
...
请根据上述{('项目结构与' if project_block else '')}变更内容，分析并给出需要测试的范围建议（直接给出结论，无需重复原文）。建议结合项目内相关模块，使测试范围更全面。输出格式：分条或分段均可。"""
    ...
    try:
        return _call_cursor(api_key, base, model_name, system_prompt, user_prompt)
